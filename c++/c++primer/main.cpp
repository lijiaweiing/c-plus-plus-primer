...
#include<string>
using namespace std;
unsigned int vowel_cnt,space_cnt,tab_cnt,enter_cnt,ff_cnt,fl_cnt,fi_cnt;
char alphabet;
vector<char>temp;
int main()
{
  while(cin>>alphabet&&alphabet!='999')
  {
  ...
  for(unsigned int i=0;i<temp.size();++i)
  {
    if(temp[i]=='a'||temp[i]=='e'||temp[i]=='i'||temp[i]=='o'||temp[i]=='u'||temp[i]=='A'||temp[i]=='E'||temp[i]=='I'||temp[i]=='O'||temp[i]=='U')
		...
		++enter_cnt;
	else if(temp[i]=='f'&&temp[i+1]=='f')
		++ff_cnt;
	else if(temp[i]=='f'&&temp[i+1]=='i')
		++fi_cnt;
  ...
  cout<<vowel_cnt<<endl<<space_cnt<<endl<<tab_cnt<<endl<<enter_cnt<<endl<<ff_cnt<<endl<<fi_cnt<<endl<<fl_cnt;
